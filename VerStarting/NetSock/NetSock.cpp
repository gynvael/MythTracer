...
NetSock::Listen(unsigned short port, const char *bindip)
{
  ...
  this->bindip = desc.sin_addr.s_addr;
  this->bindport = port;
...
NetSock::GetStrIP()
{
  ...
  in_addr in_addr_ip;
  memcpy(&in_addr_ip, &this->ip, 4);

  strncpy(this->str_ip, inet_ntoa(in_addr_ip), 16);
  this->str_ip[15] = '\0';
  return this->str_ip;
}
...
NetSock::GetStrBindIP()
{
  ...
  in_addr in_bind_ip;
  memcpy(&in_bind_ip, &this->bindip, 4);

  strncpy(this->str_bindip, inet_ntoa(in_bind_ip), 16);
